...
object CoreModule {
    ...
    /**
     * Configured now even though nothing calls it yet, so the handoff to the
     * real network layer is only about writing repositories.
     *
     * OkHttp is the engine because the Supabase Kotlin SDK needs a Ktor engine,
     * and one HTTP stack means one place for timeouts, retries and, later,
     * token refresh.
     */
    @Provides
    @Singleton
    fun provideHttpClient(json: Json): HttpClient = HttpClient(OkHttp) {
        // Non-2xx becomes an exception, so repositories can wrap calls in
        // runCatching and return Result rather than inspecting status codes.
        expectSuccess = true

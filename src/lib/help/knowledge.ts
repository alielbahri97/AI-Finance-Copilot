...
3. To recategorize existing transactions, use the [Transactions](/transactions) page: tap a row for a quick label sheet, change the category inline, or select multiple rows for bulk edits. For a short teach session, open [Teach categories](/transactions/categorize) — it shows about eight of the biggest unlabeled transactions (not the whole backlog) with large category buttons so Ballast learns for next time.`,
  },
  {
    id: "transactions",
